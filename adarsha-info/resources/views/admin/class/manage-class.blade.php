...
@section('body')
    <div class="breadcrumb-wrapper">
	    <div class="card-header">
            <h4 class="text-danger text-bold">  Manage Class
                <a href="{{route('class')}}" class="btn btn-danger btn-sm float-right">
                    <span class="mdi-hand-pointing-right"> </span>Add Class
                </a>
            </h4>
        </div>
    </div>
							...
							<tbody>
								...
								@foreach($getClasses as $getClass)
									<tr >
										...
										<td class="text-center">{{$getClass->class_name}}</td>
										<td class="text-center">{{$getClass->status == 1 ? 'Active' : 'Inactive'}}</td>
										<td class="text-center">
											@if($getClass->status == 1)
                                                <a class="btn btn-info btn-sm" href="{!! route('inactive-class', ['id'=>$getClass->id]) !!}" title="Active">
                                                    <span class="mdi mdi-arrow-up-bold"></span>
                                                </a>
                                            @else
                                                <a class="btn btn-warning btn-sm" href="{!! route('active-class', ['id'=>$getClass->id]) !!}" title="Inactive">
