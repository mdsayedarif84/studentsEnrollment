...
@section('body')
                ...
                <div class="card-body">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="text-danger text-bold"> Edit Class
                                <a href="{{route('manage-class')}}" class="btn btn-danger btn-sm float-right">
                                    <span class="mdi-hand-pointing-right"> </span>Manage Class</a>
                            </h4>
                        </div>
                        <div class="card-body">
                            <form action="{{route('class-update')}}" method="POST" id="editData" enctype="multipart/form-data"  class="form-horizontal" >
                                @csrf
                                <div class="card-body">
                                    <div class="form-group row">
                                        <label for="name" class="col-sm-4 col-form-label text-right">Class Name</label>
                                        <div class="col-sm-8">
                                            <input type="text" name="class_name" value="{{$editClass->class_name}}" class="form-control @error('class_name') is-invalid @enderror" id="name" placeholder="Enter name">
                                            <input type="hidden" name="class_id" value="{{$editClass->id}}" class="form-control " >
                                            @error('class_name')
    ...
    <script>
        document.forms['editData'].elements['status'].value = '{!! $editClass->status !!}';
    </script>
@endsection